...
if [[ ! -z "${DEST_HOST}" ]]; then
  echo "Adding iptables rule for import DNAT"
  iptables -t nat -I PREROUTING \-d "${POD_IP}" -j DNAT --to-destination "$(tailscale --socket=/tmp/tailscaled.sock ip -4 ${DEST_HOST})"
  iptables -t nat -A POSTROUTING -o tailscale0 -j MASQUERADE
else 
  echo "Adding iptables rule for export DNAT"
  var_name=`echo $"${SERVICE}_SERVICE_HOST" | tr '[:lower:]' '[:upper:]'`
  DEST_IP=`eval echo $var_name`
  echo iptables -t nat -I PREROUTING -d "$(tailscale --socket=/tmp/tailscaled.sock ip -4)" -j DNAT --to-destination "${DEST_IP}"
